...
def book_slot(request):
    ...
    if request.method == 'POST':
        ...
        tx_id = f"TXN-{str(booking.booking_id).split('-')[0].upper()}"
        return redirect(f"/paypal/dummy/{booking.booking_id}/?tx={tx_id}")
...
def dummy_paypal_payment(request, booking_id):
    ...
    tx_id = request.GET.get('tx') or request.POST.get('tx_id') or f"TXN-{str(booking.booking_id).split('-')[0].upper()}"
    ...
    if request.method == 'POST':
        ...
        messages.success(request, f'Dummy card payment successful! Transaction: {tx_id}')
        return redirect('booking_confirmation', booking_id=booking.booking_id)

    return render(request, 'parking/dummy_paypal.html', {'booking': booking, 'amount': amount, 'tx_id': tx_id})
